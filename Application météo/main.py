...
async def choix_ville(event):
    ...
    if previsions['name'] == select_html.value:
        ...
        if affichage_previsions.value == "Légères Chutes De Neige" or affichage_previsions.value == "Chutes De Neige" or affichage_previsions.value == "Pluie Légère Et Neige":
            ...
            animation.innerHTML = animation.value
            '''
        if select_html.value == "Bavay":
            animation.src = "assets/bavay.jpg"
            animation.innerHTML = animation.value
            '''
...
async def composition(event):
    ...
    compo.innerHTML = f"""Co : {composants['list'][0]['components']['co']}\n
    No : {composants['list'][0]['components']['no']}\n
    No2 : {composants['list'][0]['components']['no2']}\n
    O3 : {composants['list'][0]['components']['o3']}\n
    So2 :  {composants['list'][0]['components']['so2']}\n
    Pm2_5 :  {composants['list'][0]['components']['pm2_5']}\n 
    Pm10 : {composants['list'][0]['components']['pm10']}"""
    #compo = compo.replace("\n", "<br>") retour à la ligne -> "retour chariot?"

#Insertion d'un gif <iframe src="https://giphy.com/embed/eiMzTkBCN4lGg" width="480" height="480" style="" frameBorder="0" class="giphy-embed" allowFullScreen></iframe><p><a href=>via GIPHY</a></p>